...
$cat_result = mysqli_query($conn, "SELECT * FROM categories WHERE slug = '$slug' AND status = 1");
...
switch($sort) {
    case 'price_asc': $order = "COALESCE(sale_price, price) ASC"; break;
    case 'price_desc': $order = "COALESCE(sale_price, price) DESC"; break;
...
$count_result = mysqli_query($conn, "SELECT COUNT(*) as count FROM products WHERE $where");
$total = 0;
if ($count_result) {
...
<div class="bg-gradient-to-br from-gold-50 via-white to-gold-50 py-12">
    <div class="container mx-auto px-4">
        <!-- Breadcrumb -->
        <nav class="flex mb-8 text-sm animate-fadeInUp">
            <a href="index.php" class="text-gold-600 hover:text-gold-700 font-semibold">Trang chủ</a>
            <span class="mx-2 text-gray-400">/</span>
            <span class="text-gray-600"><?php echo $category['name']; ?></span>
        </nav>

        <!-- Category Header -->
        <div class="text-center mb-12 animate-fadeInUp">
            <h1 class="text-5xl font-bold text-gray-900 mb-4"><?php echo $category['name']; ?></h1>
            <?php if($category['description']): ?>
            <p class="text-gray-600 text-lg max-w-3xl mx-auto"><?php echo $category['description']; ?></p>
            <?php endif; ?>
            <div class="w-24 h-1 bg-gradient-to-r from-gold-600 to-gold-400 mx-auto mt-4"></div>
            <p class="text-gold-600 font-semibold mt-4"><?php echo $total; ?> sản phẩm</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-12">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="category.php?slug=<?php echo $slug; ?>" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-semibold">
        ...
        <div class="flex justify-center items-center space-x-2">
            ...
            if ($page > 1): ?>
            <a href="?<?php echo substr($query_string, 1); ?>&page=<?php echo $page - 1; ?>" class="px-4 py-2 bg-white rounded-lg border-2 border-gray-200 hover:border-gold-600 hover:text-gold-600 transition font-semibold"><i class="fas fa-chevron-left"></i></a>
            <?php endif; for ($i = 1; $i <= $total_pages; $i++): if ($i == 1 || $i == $total_pages || ($i >= $page - 2 && $i <= $page + 2)): ?>
            <a href="?<?php echo substr($query_string, 1); ?>&page=<?php echo $i; ?>" class="px-4 py-2 rounded-lg border-2 font-semibold transition <?php echo $page == $i ? 'bg-gradient-to-r from-gold-600 to-gold-400 text-white border-gold-600' : 'bg-white border-gray-200 hover:border-gold-600 hover:text-gold-600'; ?>"><?php echo $i; ?></a>
            <?php elseif ($i == $page - 3 || $i == $page + 3): echo '<span class="px-2 text-gray-400">...</span>'; endif; endfor; if ($page < $total_pages): ?>
            <a href="?<?php echo substr($query_string, 1); ?>&page=<?php echo $page + 1; ?>" class="px-4 py-2 bg-white rounded-lg border-2 border-gray-200 hover:border-gold-600 hover:text-gold-600 transition font-semibold"><i class="fas fa-chevron-right"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        <?php else: ?>
        <div class="text-center py-20 bg-white rounded-2xl shadow-lg">
            ...
            <h3 class="text-2xl font-bold text-gray-900 mb-4">Chưa có sản phẩm trong danh mục này</h3>
            <a href="products.php" class="inline-flex items-center px-8 py-3 bg-gradient-to-r from-gold-600 to-gold-400 text-white rounded-full hover:shadow-lg transition-all hover:scale-105 font-semibold"><i class="fas fa-tags mr-2"></i>Xem tất cả sản phẩm</a>
        </div>
        <?php endif; ?>
    </div>
</div>
